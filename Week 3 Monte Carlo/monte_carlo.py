...
GAMMA = 0.9
EPSILON = 0.2   # Probability of taking random action in Epsilon Greedy Selection Algorithm
...
def monte_carlo(grid):
    ...
    Q = {}
    returns = {}    # dictionary of state -> list of return we've received
    ...
    for t in range(N_EPISODES):
        ...
        for s, a, G in states_actions_returns:
            ...
            if sa not in seen_state_action_pairs:
                returns[sa].append(G)
                old_q = Q[s][a]
                # the new Q[s][a] is the simple mean of all our returns for that (state, action)
